...
def extract_features(mesh):
    ...
    with np.errstate(divide='raise'):
        try:
            for extractor in [dihedral_angle, symmetric_opposite_angles, symmetric_ratios, edge_ratios]:
...
def extract_features1(mask,mesh):
    features=[]
    edge_points=get_edge_points(mesh)
    set_edge_lengths(mesh, edge_points)
    with np.errstate(divide='raise'):
...
def edge_ratios(mesh, edge_points):

    ratios_a = get_edge_ratios(mesh, edge_points, 0)
    ratios_b = get_edge_ratios(mesh, edge_points, 3)
    ratios = np.concatenate((np.expand_dims(ratios_a, 0), np.expand_dims(ratios_b, 0)), axis=0)
    return np.sort(ratios, axis=0)
...
def get_edge_ratios(mesh, edge_points, side):

    edges_lengths_oa = np.linalg.norm(mesh.vs[edge_points[:, side // 2]] - mesh.vs[edge_points[:,side // 2 + 2]],
                                   ord=2, axis=1)
    edges_lengths_ob= np.linalg.norm(mesh.vs[edge_points[:, 1 - side // 2]] - mesh.vs[edge_points[:,side // 2 + 2]],
                                   ord=2, axis=1)
   #
    #if edges_lengths_ob == 0.0:
    return edges_lengths_oa/edges_lengths_ob
