...
class Mesh:
    ...
    def __init__(self, file=None, opt=None, hold_history=False, export_folder=''):
        self.vs = self.faces=self.v_mask = self.filename = self.features = self.edge_areas = None
    ...
    # 融合点(相当于把这个边去掉了)
    def merge_vertices(self, edge_id, new_vertice):

        self.remove_edge(edge_id)  # 将edge_id的两个点的邻边中去掉edge_id
        edge = self.edges[edge_id]
        v_a = self.vs[edge[0]]  # v_a和v_b就是edge的两个点

        v_b = self.vs[edge[1]]
        # update pA
        # v_a.__iadd__(v_b)  # 这是什么意思，是加？
        # v_a.__itruediv__(2)  # 这个是除法的意思么?
        v_a[0] = new_vertice[0]
        v_a[1] = new_vertice[1]
        v_a[2] = new_vertice[2]
        ...
        self.v_mask[edge[1]] = False
        mask = self.edges == edge[1]
        self.ve[edge[0]].extend(self.ve[edge[1]])  # extend() 函数用于在列表末尾一次性追加另一个序列中的多个值
        # 由于将edge[0]和edge[1]融合起来了，那么就将edge[1]的邻边也都加到了edge[0]这
        self.edges[mask] = edge[0]
